...
public class NPCToken extends Token
{
    ...
    protected String buildPowerHtml(Power p)
    {
        ...
        // display action type
        if (p.getActionType() != "")
            html += p.getActionType();

        if (p.getUsage().contains("recharge"))
            html += "; " + p.getUsage();
        html += "</p>";
        ...
        html = html
                .replace(
                        ...
                        "<span style=\""
                                + linkFont
                                + "\">[r:macroLink(\"recharge\", \"RechargeRoll@Lib:Blakey\", \"all\", \"Recharge=4;Power="
                                + p.getName()
                                + "\", currentToken())]</span> 456");
        html = html
                .replace(
                        "recharge 5",
                        "<span style=\""
                                + linkFont
                                + "\">[r:macroLink(\"recharge\", \"RechargeRoll@Lib:Blakey\", \"all\", \"Recharge=5;Power="
                                + p.getName()
                                + "\", currentToken())]</span> 56");
        html = html
                .replace(
                        "recharge 6",
                        "<span style=\""
                                + linkFont
                                + "\">[r:macroLink(\"recharge\", \"RechargeRoll@Lib:Blakey\", \"all\", \"Recharge=6;Power="
                                + p.getName() + "\", currentToken())]</span> 6");
    ...
    private String addMacroLinks(Power p)
    {
        ...
        while (matcher.find())
        {
            ...
            String replacement = "[r:macroLink(\""
                    ...
                    + attack + ";Defence=" + defence + ";Power=" + p.getName()
                    + ";DamageDice=" + dice + "plus" + plus + ";Usage="
                    + p.getUsage() + ";DamageType=" + type
                    + "\", currentToken())]";
        ...
        pattern = Pattern.compile("(([0-9]+)\\s([a-z]+\\s)*damage)");
        matcher = pattern.matcher(html);
        sb = new StringBuffer();
        while (matcher.find())
        {
        ...
        // if this power has no attack specified, add a UsePower macro link to
        // use it.
        if (!foundAttack)
        {
            html += "<br/>[r:macroLink(\"Use " + p.getName()
                    + "\", \"UsePower@Lib:Blakey\", \"all\", \"Power="
                    + p.getName().replace(" ", "+") + ";Usage="
    ...
    public void saveXML()
    {
        ...
        try
        {
            ...
            writer.write("    <Values>\n");
            writer.write("      <AbilityScoreNumber xsi:type=\"AbilityScoreNumber\" FinalValue=\""
                    + me.getAttributes().get(Attribute.Strength) + "\">\n");
            writer.write("        <Name>Strength</Name>\n");
            writer.write("      </AbilityScoreNumber>\n");
            writer.write("      <AbilityScoreNumber xsi:type=\"AbilityScoreNumber\" FinalValue=\""
                    + me.getAttributes().get(Attribute.Constitution) + "\">\n");
            writer.write("        <Name>Constitution</Name>\n");
            writer.write("      </AbilityScoreNumber>\n");
            writer.write("      <AbilityScoreNumber xsi:type=\"AbilityScoreNumber\" FinalValue=\""
                    + me.getAttributes().get(Attribute.Dexterity) + "\">\n");
            writer.write("        <Name>Dexterity</Name>\n");
            writer.write("      </AbilityScoreNumber>\n");
            writer.write("      <AbilityScoreNumber xsi:type=\"AbilityScoreNumber\" FinalValue=\""
                    + me.getAttributes().get(Attribute.Intelligence) + "\">\n");
            writer.write("        <Name>Intelligence</Name>\n");
            writer.write("      </AbilityScoreNumber>\n");
            writer.write("      <AbilityScoreNumber xsi:type=\"AbilityScoreNumber\" FinalValue=\""
                    + me.getAttributes().get(Attribute.Wisdom) + "\">\n");
            writer.write("        <Name>Wisdom</Name>\n");
            writer.write("      </AbilityScoreNumber>\n");
            writer.write("      <AbilityScoreNumber xsi:type=\"AbilityScoreNumber\" FinalValue=\""
                    + me.getAttributes().get(Attribute.Charisma) + "\">\n");
            ...
            writer.write("    <Values>\n");
            writer.write("      <SimpleAdjustableNumber xsi:type=\"SimpleAdjustableNumber\" FinalValue=\""
                    + me.getAC() + "\">\n");
            ...
            if (!myNpc.getImprovedSkills().isEmpty())
            {
                for (String skillName : myNpc.getImprovedSkills().split(", "))
                {
                    ...
                    writer.write("      <SkillNumber xsi:type=\"SkillNumber\" FinalValue=\""
                            + me.getSkills().get(Skill.valueOf(skillName))
                            + "\">\n");
            ...
            // Compendium sometimes adds another word to the Type after a comma,
            // so split that off too
            ...
            if (me.getRegeneration().isEmpty())
                ...
                writer.write("  <Regeneration FinalValue=\""
                        + me.getRegeneration() + "\">\n");
            ...
            writer.write("  <Description>" + me.getRealDescription()
                    + "</Description>\n");
            // Keywords
            ...
            if (!me.getAura().isEmpty())
            {
                ...
                writer.write("          <Value>" + me.getAuraRange()
                        + "</Value>\n");
                writer.write("        </DefaultBonus>\n");
                writer.write("      </Range>\n");
                writer.write("      <Details>" + me.getAuraDescription()
                        + "</Details>\n");
                writer.write("      <Name>" + me.getAura() + "</Name>\n");
                writer.write("      <Type>Trait</Type>\n");
                writer.write("      <IsBasic>false</IsBasic>\n");
                // Keywords aren't parsed by Masterplan for auras, ignore for
                // now
                // writer.write("      <Keywords>\n");
                // writer.write("        <ObjectReference>\n");
                // writer.write("          <ReferencedObject>\n");
                // writer.write("            <Name>" + (me.getAuraKeywords() !=
                // null ? me.getAuraKeywords() : "") + "</Name>\n");
                // writer.write("          </ReferencedObject>\n");
                // writer.write("        </ObjectReference>\n");
                // writer.write("      </Keywords>\n");
            ...
            {
                if (p.getActionType().isEmpty()
                        || p.getActionType().toLowerCase().contains("trait"))
                {
                    // if no action type, or contains the word trait, it is a
                    // Trait
                    writer.write("    <MonsterTrait xsi:type=\"MonsterTrait\">\n");
                    ...
                    writer.write("        </DefaultBonus>\n");
                    writer.write("      </Range>\n");
                    writer.write("      <Details>" + p.getDetail()
                            + "</Details>\n");
                    writer.write("      <Name>" + p.getName() + "</Name>\n");
                    writer.write("      <Type>Trait</Type>\n");
                    writer.write("      <IsBasic>false</IsBasic>\n");
                    // writer.write("      <IsBasic>" + (p.getIsBasicAtk() ?
                    // "true" : "false") + "</IsBasic>\n");
                    writer.write("      <Keywords>\n");
                ...
                else
                {
                    ...
                    // Notes: Masterplan does a pretty crappy job of parsing
                    // powers. Adventure Tools has a <FailedSavingThrows> tag
                    // for obvious uses,
                    // but Masterplan ignores it. It does read and parse the
                    // <Hit>, <Miss>, <Effect> tags though...but all it does is
                    // append the
                    // <Damage> and <Description> of each section along with
                    // whatever <Name> you give in front of it all shoved into
                    // the Details tab.
                    // For my uses I'll drop <FailedSavingThrows>, and I'm going
                    // to just put everything into <Hit> tag under <Description>
                    // for now.
                    // By not parsing the damage out into <Damage> tags I get an
                    // extra space after the text from the <Name> tag but I can
                    // manually
                    // edit that around since I have to probably touch every
                    // power (especially auras) on these after creation just to
                    // be sure it worked.
                    // Masterplan doesn't seem to handle the new conditionals
                    // from MM3 files either, so I can skip that too and I'll
                    // just have to type
                    // it in by hand.
                    writer.write("    <MonsterPower xsi:type=\"MonsterPower\">\n");
                    writer.write("      <Action>" + p.getActionType()
                            + "</Action>\n");
                    writer.write("      <Usage>" + p.getUsage() + "</Usage>\n");
                    writer.write("      <Attacks>\n");
                    writer.write("        <MonsterAttack>\n");
                    if (!p.getRange().trim().isEmpty())
                        writer.write("          <Range>" + p.getRange().trim()
                                + "</Range>\n");
                    // If there is no target defense print an Effect node with a
                    // blank AttackBonuses node
                    if (p.getAtkDefense().isEmpty())
                    {
                        writer.write("          <Effect>\n");
                        writer.write("            <Name>Effect</Name>\n");
                        writer.write("            <Description>"
                                + p.getDetail().trim() + "</Description>\n");
                        writer.write("          </Effect>\n");
                        writer.write("          <AttackBonuses />\n");
                    }
                    else
                    {
                        ...
                        writer.write("          </Hit>\n");
                        writer.write("          <AttackBonuses>\n");
                        writer.write("            <MonsterPowerAttackNumber FinalValue=\""
                                + p.getAtkBonus() + "\">\n");
                        writer.write("              <Defense>\n");
                        writer.write("                <ReferencedObject>\n");
                        // Have to write Will instead of Willpower for
                        // Masterplan parsing
                        writer.write("                  <DefenseName>"
                                + (p.getAtkDefense().startsWith("Will") ? "Will"
                                        : p.getAtkDefense())
                                + "</DefenseName>\n");
                        writer.write("                </ReferencedObject>\n");
                        writer.write("              </Defense>\n");
            ...
            writer.write("  </Powers>\n");
            // Initiative
            writer.write("  <Initiative xsi:type=\"SimpleAdjustableNumber\" FinalValue=\""
                    + me.getInitiative() + "\">\n");
            writer.write("    <Name>Initiative</Name>\n");
            writer.write("  </Initiative>\n");
            // Hitpoints
            writer.write("  <HitPoints xsi:type=\"SimpleAdjustableNumber\" FinalValue=\""
                    + me.getHP() + "\">\n");
            ...
            // Speeds have a <Details> block for adding things like (hover), but
            // you can cheat and put it all in Name
            writer.write("  <Speeds>\n");
            ...
            for (String immunes : me.getImmune().split(", "))
            {
                ...
                writer.write("      <ReferencedObject>\n");
                writer.write("        <Name>"
                        + immunes.substring(0, 1).toUpperCase()
                        + immunes.substring(1) + "</Name>\n");
                writer.write("      </ReferencedObject>\n");
                writer.write("    </ObjectReference>\n");
            }
            writer.write("  </Immunities>\n");
            ...
            // Sometimes the compendium swaps Resists with Immunes on accident,
            // like the Atropal, token code handles that but I don't bother for
            // Masterplan
            for (String resists : me.getResistances().split(", "))
            {
                ...
                writer.write("        <Name>"
                        ...
                        + resistSplit[1].substring(1) + "</Name>\n");
                writer.write("      </ReferencedObject>\n");
